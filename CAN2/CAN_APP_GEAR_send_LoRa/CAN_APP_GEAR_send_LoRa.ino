...
// -----------------------------
// Gear decode helpers for 0x151
// -----------------------------
bool isIgnoreGearFrame(const uint8_t *buf) {
  // Known transient frame during shifts:
  // 10 00 00 00 60 00 00 00
  return (buf[1] == 0x00 && buf[4] == 0x60);
}

String decodeGear151(const uint8_t *buf) {
  switch (buf[4]) {
    case 0x00: return "P";
    case 0x20: return "R";
    case 0x40: return "N";
    case 0x60:
      // Accept Drive only on valid frame, not the transient one
...
void loop() {
  ...
  while (digitalRead(CAN_INT_PIN) == LOW) {
    ...
    // -------------------------
    // Raw gear frame on ID 0x151
    // -------------------------
    if (rxId == 0x151) {
      if (!isIgnoreGearFrame(buf)) {
        String gear = decodeGear151(buf);
        if (gear != "?") {
          currentGear = gear;
        }
      }
  ...
  // Output / LoRa serial line every 200 ms
