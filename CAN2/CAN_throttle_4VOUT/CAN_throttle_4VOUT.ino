...
// Output values
float minV = 0.780f;
float vin = 0.780f;          // mapped continuous voltage from APP
int pwm = 0;
int pwmHalf = 0;
...
// --------------------------------------
// Map CAN APP % to continuous 0.7V..4V
// --------------------------------------
float mapPedalToVoltage(float appRaw) {
  ...
  return minV + x * (4.0f - minV); // 0V to 4V
}
